...
class MomentumTrader(EA):
    ...
    def is_momentum_trade(self, asset):
        ...
        prices_df.ta.macd(close='Close', fast=12, slow=26, signal=9, append=True) #12, 26, 9
        ...
        alpha = 0.5
        dt = 1 # delta t
        if (macd[-1]-macds[-1]) * (macd[-2]-macds[-2]) < 0: # i.e. if MACD crosses signal as TRUE only if the one that was up is now down
